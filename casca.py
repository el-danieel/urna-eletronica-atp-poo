...
def press_key(key):
    ...
    if key == 'Branco':
        ...
        input_number += key
        input_label.config(text=input_number)  # Atualiza o Label com o número atual
        print(f"Tecla pressionada: {key}, Número atual: {input_number}")
        if len(input_number) == 2: 
            ver_candidato_info(input_number)

    elif key == 'Corrigir':  # Limpa o número caso Corrigir seja pressionado
        input_number = ""
        input_label.config(text=input_number)
...
top_img = teclas(nums.get('top', 'teclas/top-2.png'), size=(315, 130))
...
input_label = tk.Label(left_frame, text="", bg='white', fg='black', font=('Arial', 24), anchor="center")
input_label.pack(side="top", padx=10, pady=20)
